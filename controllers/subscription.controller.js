...
// Enhanced search functionality for subscriptions
export const searchSubscriptions = async (req, res, next) => {
    try {
        ...
        // Payment method filter (case-insensitive partial match)
        if (paymentMethod) {
        ...
        const totalCount = await Subscription.countDocuments(searchQuery);
        ...
        res.status(200).json({
            success: true,
            message: `Found ${subscriptions.length} subscription(s) matching your criteria`,
            data: {
                subscriptions,
                pagination: {
                    currentPage: pageNum,
                ...
                filters: {
                    ...
                    renewalDateRange: { start: renewalStartDate, end: renewalEndDate }
                }
            }
        });
